...
def log_body_markers(df, i, marker_radius):
    for idx in range(1, 6):
        rr.log(
            ...
            rr.Points3D(
                [df[f"fb{idx}x"].iloc[i], df[f"fb{idx}z"].iloc[i], df[f"fb{idx}y"].iloc[i]],
                colors=[0, 255, 0],
                radii=[marker_radius],
            ),
        )
...
def log_wing_markers(df, i, wing, marker_radius):
    ...
    rr.log(
        ...
        rr.Points3D(
            [df[f"fb{wing[0]}w{'x'}"].iloc[i], df[f"fb{wing[0]}w{'z'}"].iloc[i], df[f"fb{wing[0]}w{'y'}"].iloc[i]],
            colors=color,
            radii=[marker_radius],
        ),
    )
    for idx in [1, 2, 3]:
        rr.log(
            ...
            rr.Points3D(
                [df[f"fb{wing[0]}w{idx}x"].iloc[i], df[f"fb{wing[0]}w{idx}z"].iloc[i], df[f"fb{wing[0]}w{idx}y"].iloc[i]],
                colors=color,
                radii=[marker_radius],
            ),
        )
...
def log_wing_strips(df, i, wing, line_radius):
    ...
    rr.log(
        ...
        rr.LineStrips3D(
            [
                [df[f"fb{wing[0]}w1x"].iloc[i], df[f"fb{wing[0]}w1z"].iloc[i], df[f"fb{wing[0]}w1y"].iloc[i]],
                [df[f"fb{wing[0]}w2x"].iloc[i], df[f"fb{wing[0]}w2z"].iloc[i], df[f"fb{wing[0]}w2y"].iloc[i]],
                [df[f"fb{wing[0]}w3x"].iloc[i], df[f"fb{wing[0]}w3z"].iloc[i], df[f"fb{wing[0]}w3y"].iloc[i]],
            ],
...
def log_mirrored_wing_markers(df, i, wing, marker_radius, body_1_shifted, body):
    color = [255, 0, 0] if wing == "right" else [0, 0, 255]
    A = np.array([df[f"fb{wing[0]}wx"].iloc[i], df[f"fb{wing[0]}wz"].iloc[i], df[f"fb{wing[0]}wy"].iloc[i]])
    B = body_1_shifted
    C = body
    for idx in [1, 2, 3]:
        P = np.array([df[f"fb{wing[0]}w{idx}x"].iloc[i], df[f"fb{wing[0]}w{idx}z"].iloc[i], df[f"fb{wing[0]}w{idx}y"].iloc[i]])
...
if __name__ == "__main__":
    ...
    names = [
        "time", "fbqx", "fbqy", "fbqz", "fbqw", "fbx", "fby", "fbz",
        "fb1x", "fb1y", "fb1z", "fb2x", "fb2y", "fb2z", "fb3x", "fb3y", "fb3z",
        "fb4x", "fb4y", "fb4z", "fb5x", "fb5y", "fb5z",
        "fbrwqx", "fbrwqy", "fbrwqz", "fbrwqw", "fbrwx", "fbrwy", "fbrwz",
        "fbrw1x", "fbrw1y", "fbrw1z", "fbrw2x", "fbrw2y", "fbrw2z", "fbrw3x", "fbrw3y", "fbrw3z",
        "fblwqx", "fblwqy", "fblwqz", "fblwqw", "fblwx", "fblwy", "fblwz",
        "fblw1x", "fblw1y", "fblw1z", "fblw2x", "fblw2y", "fblw2z", "fblw3x", "fblw3y", "fblw3z",
    ]
    df = pd.read_csv(
    ...
    for i in range(len(df)):
        ...
        quat = np.array([df["fbqw"].iloc[i], df["fbqx"].iloc[i], df["fbqy"].iloc[i], df["fbqz"].iloc[i]])  # w, x, y, z
        ...
        left_wing_body = np.array([df["fblwx"].iloc[i], df["fblwz"].iloc[i], df["fblwy"].iloc[i]])
        right_wing_body = np.array([df["fbrwx"].iloc[i], df["fbrwz"].iloc[i], df["fbrwy"].iloc[i]])
        wing_body_distance = 0.9 * wing_body_distance + 0.1 * np.linalg.norm(left_wing_body - right_wing_body)
